...
from gameRule import checkMoveValidation, end_game_check, Getstep
...
def main(teams, path, node_num, seed):
    # use cmd input
    '''for i in range(2):
    ...
    if (not success):
        ...
        winner = battle(teams, node_num, seed)
...
if __name__ == "__main__":
    ...
    player1_win_cnt_first_hand = 0
    player1_win_cnt_second_hand = 0
    
    p = tqdm(total=epoch)
    for i in range(epoch):
        node_num = random.randint(13, 34)
        seed = random.randint(1, 1001)
        p.write(f"node_num = {node_num}, seed = {seed}")

        winner = main(teams, path, node_num, seed)
        if winner == 1:
            player1_win_cnt_first_hand += 1
        else:
            p.write("Player1 First Hand Lose")
        ...
        winner = main(teams, path, node_num, seed)
        if winner == 2:
            player1_win_cnt_second_hand += 1
        else:
            p.write("Player1 Second Hand Lose")
    ...
    print("---------------")
    print(f"Total matches = {2 * epoch}")
    print(f"Player1 First Hand Win Count: {player1_win_cnt_first_hand}")
    print(f"Player1 Second Hand Win Count: {player1_win_cnt_second_hand}")
    print(f"Player1 First Hand Win Rate: {player1_win_cnt_first_hand / epoch}")
    print(f"Player1 Second Hand Win Rate: {player1_win_cnt_second_hand / epoch}")
    print(f"Player1 Average Win Rate: {(player1_win_cnt_first_hand + player1_win_cnt_second_hand) / (2 * epoch)}")
    print("---------------")
