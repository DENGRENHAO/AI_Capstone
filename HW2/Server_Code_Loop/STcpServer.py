...
def _WaitConnection(indexPlayer, idTeam, flagDirectlySpawn, pathExe):
    ...
    if (not flagDirectlySpawn):
        ...
        if (flagSuccess):
            ...
            return True
    # spawn process
    # subprocess.Popen(["taskkill", "/im", "Sample.exe"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    if (len(pathExe) != 0):
        subprocess.Popen([pathExe], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    (flagSuccess, socketClient) = _TryAccept(idTeam)
    if (flagSuccess):
        socketPlayer[indexPlayer] = socketClient
    else:
...
def _CleanUpPlayer():
    ...
    for i in range(2):
        ...
        subprocess.Popen(["taskkill", "/im", pathExe[i]], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
